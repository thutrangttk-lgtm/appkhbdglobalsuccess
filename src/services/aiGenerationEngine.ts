...
`;
  ...
  private static generateLocalSmartPlan(
    ...
    return {
      id: docId,
      title: `Lesson Plan Grade ${lesson.grade} - ${lesson.unit !== undefined ? 'Unit ' + lesson.unit : lesson.contentType || 'Section'} - ${lesson.title}`,
      grade: lesson.grade,
      week: lesson.week,
      unit: lesson.unit ?? 0,
      ...
      header: {
        ...
        teacherName,
        grade: lesson.grade,
        unitNumber: lesson.unit ?? 0,
        unitTitle: lesson.unitTitle ?? (lesson.contentType || 'Special Section'),
